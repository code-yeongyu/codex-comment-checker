...
import { type CheckerEdit, type CheckerToolInput, isRecord, toHookInput } from "./core.js";
...
export function createCommentCheckerMcpHandler(options: McpHandlerOptions = {}) {
...
export async function runMcpStdioServer(): Promise<void> {
	...
	for await (const line of lines) {
		...
		if (!isJsonRpcRequest(parsed)) continue;
...
async function callTool(params: unknown, runner: CommentCheckerRunner): Promise<ToolCallResult> {
...
function commentCheckToolInput(filePath: string, argumentsObject: Record<string, unknown>): CheckerToolInput {
...
function toolNameForInput(toolInput: CheckerToolInput): "Write" | "Edit" | "MultiEdit" {
	if (toolInput.content !== undefined) return "Write";
	if (toolInput.edits !== undefined) return "MultiEdit";
	return "Edit";
}

function getMcpEdits(value: unknown): CheckerEdit[] {
...
function commentCheckToolDefinition(): JsonValue {
	return {
		name: "comment_check",
		title: "Comment Check",
		description:
			"Run comment-checker against proposed file content or edits. Use after writing code comments or before finalizing edits.",
		inputSchema: {
			type: "object",
			properties: {
				cwd: {
...
function success(id: string | number | null, result: JsonValue): JsonRpcResponse {
	return {
		jsonrpc: "2.0",
		id,
		result,
